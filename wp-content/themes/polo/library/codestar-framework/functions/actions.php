...
if ( ! function_exists( 'csf_get_icons' ) ) {
  function csf_get_icons() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'csf_icon_nonce' ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid nonce verification.', 'polo' ) ) );
    }
    ...
    if ( ! empty( $icon_lists ) ) {
      ...
      echo '<div class="csf-error-text">'. esc_html__( 'No data available.', 'polo' ) .'</div>';
...
if ( ! function_exists( 'csf_export' ) ) {
  function csf_export() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'csf_backup_nonce' ) ) {
      die( esc_html__( 'Error: Invalid nonce verification.', 'polo' ) );
    }

    if ( empty( $unique ) ) {
      die( esc_html__( 'Error: Invalid key.', 'polo' ) );
    }
...
if ( ! function_exists( 'csf_import_ajax' ) ) {
  function csf_import_ajax() {
    ...
    if ( empty( $unique ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid key.', 'polo' ) ) );
    }

    if ( empty( $data ) || ! is_array( $data ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: The response is not a valid JSON response.', 'polo' ) ) );
    }
...
if ( ! function_exists( 'csf_chosen_ajax' ) ) {
  function csf_chosen_ajax() {
    ...
    if ( empty( $type ) || empty( $term ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid term ID.', 'polo' ) ) );
    }

    $capability = apply_filters( 'csf_chosen_ajax_capability', 'manage_options' );

    if ( ! current_user_can( $capability ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: You do not have permission to do that.', 'polo' ) ) );
    }
