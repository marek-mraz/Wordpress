  ...
  class CSF_Field_group extends CSF_Fields {
    ...
    public function render() {
      ...
      if ( preg_match( '/'. preg_quote( '['. $this->field['id'] .']' ) .'/', $this->unique ) ) {
        ...
        echo '<div class="csf-notice csf-notice-danger">'. esc_html__( 'Error: Field ID conflict.', 'polo' ) .'</div>';
        ...
        echo '<div class="csf-cloneable-item csf-cloneable-hidden">';
          ...
          echo '<i class="csf-cloneable-remove csf-confirm fas fa-times" data-confirm="'. esc_html__( 'Are you sure to delete this item?', 'polo' ) .'"></i>';
          echo '</div>';

          echo '<h4 class="csf-cloneable-title">';
          echo '<span class="csf-cloneable-text">';
          if( $title_number ){
            echo '<span class="csf-cloneable-title-number"></span>';
          }
          if( $title_prefix ){
            echo '<span class="csf-cloneable-title-prefix">'. esc_attr( $title_prefix ) .'</span>';
          }
          if( $title_auto ){
            echo '<span class="csf-cloneable-value"><span class="csf-cloneable-placeholder"></span></span>';
          }
          echo '</span>';
          echo '</h4>';
        ...
        if ( ! empty( $this->value ) ) {
          ...
          foreach ( $this->value as $value ) {
            ...
            echo '<div class="csf-cloneable-item">';
              ...
              echo '<span class="csf-cloneable-text">';
              if( $title_number ){
                echo '<span class="csf-cloneable-title-number">'. esc_attr( $num+1 ) .'.</span>';
              }
              if( $title_prefix ){
                echo '<span class="csf-cloneable-title-prefix">'. esc_attr( $title_prefix ) .'</span>';
              }
              if( $title_auto ){
                echo '<span class="csf-cloneable-value">' . esc_attr( $first_value ) .'</span>';
              }
              echo '</span>';
              echo '</h4>';
        ...
        echo '<div class="csf-cloneable-alert csf-cloneable-max">'. esc_html__( 'You cannot add more.', 'polo' ) .'</div>';
        echo '<div class="csf-cloneable-alert csf-cloneable-min">'. esc_html__( 'You cannot remove more.', 'polo' ) .'</div>';
