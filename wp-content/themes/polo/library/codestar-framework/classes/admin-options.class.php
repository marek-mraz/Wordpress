...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'polo' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'polo' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'polo' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'polo' );
        }
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            if( $this->args['show_form_warning'] ){
              echo '<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'polo' ) .'</div>';
            }

            if( $has_nav && $this->args['show_all_options'] ){
              echo '<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'polo' ) .'"><i class="fas fa-outdent"></i></div>';
            }

            if( $this->args['show_search'] ){
              echo '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( 'Search...', 'polo' ) .'" autocomplete="off" /></div>';
            }

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'polo' ) .'" data-save="'. esc_html__( 'Saving...', 'polo' ) .'">';
            
            if( $this->args['show_reset_section'] ){
              echo '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'polo' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'polo' ) .'">';
            }
            if( $this->args['show_reset_all'] ){
              echo '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'polo' ) : esc_html__( 'Reset', 'polo' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'polo' ) .'">';
            }
            echo '</div>';
        ...
        echo '<div class="csf-wrapper'. esc_attr( $show_all ) .'">';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              if( $has_nav ){
                echo '<div class="csf-section-title"><h3>'. $section_icon . $section_title .'</h3></div>';
              }
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                echo '<div class="csf-no-option">'. esc_html__( 'No data available.', 'polo' ) .'</div>';
          ...
          $background_html = ( $has_nav && $nav_type === 'normal' ) ? '<div class="csf-nav-background"></div>' : '';
          if($background_html != ''){
            echo wp_kses($background_html, array(
              'div' => array(
                'class' => true
              ),
            ));
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'polo' ) .'" data-save="'. esc_html__( 'Saving...', 'polo' ) .'">';
          if( $this->args['show_reset_section'] ){
            echo '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'polo' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'polo' ) .'">';
          }
          if( $this->args['show_reset_all'] ){
